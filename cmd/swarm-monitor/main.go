...
type SwarmNodeStatus struct {
	...
	// Traffic counters (SOCKS5 client-side)
...
type Status struct {
	...
	// Traffic counters (client-side SOCKS5)
...
func main() {
	flag.StringVar(&htmlDir, "html", "/opt/swarm-monitor", "путь к папке с index.html")
	flag.Parse()

	log.Printf("S.W.A.R.M. Monitor запущен на :%d (html: %s)", webPort, htmlDir)
	go pollLoop()
